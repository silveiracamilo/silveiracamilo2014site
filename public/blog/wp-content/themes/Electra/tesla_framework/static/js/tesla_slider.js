...
(function($){
    ...
    $(function(){
		...
		$('#slide_options').on('click','.tesla-option>button',function(){
			...
			t_select = t_clone.children('select').not('[name]');
			if(t_select.length){
				t_clone.children('.tesla-option:first').children('.tesla-option-container').children('input[disabled],textarea[disabled],select[disabled]').prop('disabled',false);
				t_clone.children('.tesla-option:first').children('.tesla-option-container').children('label').children('input[disabled]').prop('disabled',false); //for radio and checkbox type inputs
				//non grouped
				t_clone.children('.tesla-option:first').children('input[disabled][name]').prop('disabled',false);
			}else{
				t_clone.children('.tesla-option').children('.tesla-option-container').children('input[disabled],textarea[disabled],select[disabled]').prop('disabled',false);
				t_clone.children('.tesla-option').children('.tesla-option-container').children('label').children('input[disabled]').prop('disabled',false); //for radio and checkbox type inputs
				//for non grouped options
				t_clone.children('input[disabled],textarea[disabled],select[disabled]').prop('disabled',false);
				t_clone.children('label').children('input[disabled]').prop('disabled',false); //for radio and checkbox type inputs
			}
		...
		$('#slide_options').on('change','select:not([name])',function(){
			...
			t_disabled.find('input, textarea, select').not('.tesla-option-template input, .tesla-option-template textarea, .tesla-option-template select').not(function(){
				return $(this).parents('.tesla-option').is(function(){
					return $(this).css('display')==='none';
				});
			}).prop('disabled',true);
			t_disabled.css({display:'none'});
			t_selected.css({display:'block'}).find('input, textarea, select').not('.tesla-option-template input, .tesla-option-template textarea, .tesla-option-template select').not(function(){
				return $(this).parents('.tesla-option').is(function(){
					return $(this).css('display')==='none';
				});
			}).prop('disabled',false);
