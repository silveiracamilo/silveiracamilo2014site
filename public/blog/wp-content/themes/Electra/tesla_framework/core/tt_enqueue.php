...
class TT_ENQUEUE {
            ...
            if(strpos($js_file, '/admin/') === FALSE)
                if(basename($js_file) != self::$main_js)
                    if(strpos($js_file, 'http') !== FALSE){
                        ...
                        wp_enqueue_script('tt-external-js' . rand(), TT_THEME_URI . $js_file, array('jquery'), false, true);
        }
    ...
    public static function load_stylesheet(){
        ...
        if(!empty(self::$gfont_changer)){
            foreach(self::$gfont_changer as $gfont_changer){
                $gfont_changer = str_replace(' ', '+', $gfont_changer);
                wp_enqueue_style( 'tt-custom-font-' . $gfont_changer, "$protocol://fonts.googleapis.com/css?family=$gfont_changer");
            }
        }
     ...
     */
    public static function add_js($js_files){
        if(is_array($js_files))
            foreach ($js_files as $key => $js_file_link) {
                self::$js_files[] = $js_file_link;
            }
        else
     ...
     */
    public static function add_css($css_files){
        if(is_array($css_files))
            foreach ($css_files as $key => $css_file_link) {
                self::$css_files[] = $css_file_link;
            }
        else
