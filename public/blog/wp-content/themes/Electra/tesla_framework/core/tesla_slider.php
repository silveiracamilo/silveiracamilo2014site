...
class Tesla_slider{
		...
		if ( wp_verify_nonce( $_POST['nonce'])) {
			...
			if(array_key_exists($slider_id, $slider_options)){
				...
				foreach($posts_array as $post){
					$meta_array = get_post_meta($post->ID, 'slide_options', true);
	...
	public static function metabox_view($post, $params){
		...
		$values_array = get_post_meta($post->ID, 'slide_options', true);
	...
	private static function generate_input($type, $name, $value='', $placeholder='', $label='', $default='', $disabled=false){
		...
		switch($type){
			...
			case 'line':
				$output .= '<input type="text" name="'.$name.'" value="'.$value.'" placeholder="'.$placeholder.'" '.$disabled.' />';
				break;

			case 'text':
				$output .= '<textarea rows="1" cols="40" name="'.$name.'" placeholder="'.$placeholder.'" '.$disabled.'>'.$value.'</textarea>';
				break;

			case 'image':
				$output .= '<img src="'.(!empty($value)?wp_get_attachment_url($value):$default).'" alt="'.$placeholder.'" />';
			...
			if(is_array($option['type'])){
				if(!isset($option['group'])||$option['group']===true){
					...
					if(empty($option['multiple'])){
						...
						$result[$option_id][$key] = self::process_options( $values_array[$option_id], array( $key => $option['type'][$key] ));
					}else{
						$result[$option_id] = array();
						foreach ($values_array[$option_id] as $value) {
							$key = key($value);
				...
				if(empty($option['multiple'])){
					if(isset($values_array[$option_id])){
						if($values_array[$option_id]===''){
							...
							switch ($option['type']) {
								case 'image':
									if(is_numeric($values_array[$option_id]))
										$result[$option_id] = wp_get_attachment_url($values_array[$option_id]);
									else
										$result[$option_id] = $values_array[$option_id];
									break;
								case 'line':
									$result[$option_id] = do_shortcode($values_array[$option_id]);
									break;
								case 'text':
									$result[$option_id] = do_shortcode($values_array[$option_id]);
									break;
					...
					foreach((array)$values_array[$option_id] as $value){
						if($value===''){
							...
							switch ($option['type']) {
								case 'image':
									if(is_numeric($value))
										array_push($result[$option_id], wp_get_attachment_url($value));
									else
										array_push($result[$option_id], $value);
									break;
								case 'line':
									array_push($result[$option_id], do_shortcode($value));
									break;
								case 'text':
									array_push($result[$option_id], do_shortcode($value));
									break;
	...
	public static function get_slider_html($slider_id, $category='', $output_id = null, $post_id = null, $shortcode_parameters = array(), $query = array(), $custom = array()){
		...
		if(!array_key_exists($slider_id, $slider_options))
			...
			foreach($posts_array as $post){
				$meta = get_post_meta($post->ID, 'slide_options', true);
	...
	public static function get_related($post_id,$category){
		...
		foreach($related_array as $post){
			$meta_array = self::process_options(get_post_meta($post->ID, 'slide_options', true), $slider_options[$slider_id]['options']);
