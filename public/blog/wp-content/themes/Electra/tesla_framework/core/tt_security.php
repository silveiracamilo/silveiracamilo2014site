...
class TT_Security extends TeslaFramework {
	...
	public function check_username(){
		if ($this->username){
			...
			}elseif( !in_array( $this->username, array( 'tt_general_user','tt_other_marketplaces_user' ) ) )
				$this->state = 'corrupt';
		}else
			$this->state = 'corrupt';
		return;
	...
	private function error_message($just_warning=NULL,$custom=NULL){
		...
		if($custom)
			echo $custom;
		if ( $just_warning )
			echo '<span>WARNING :</span> We noticed some fraudulous activity with our theme or couldn\'t connect to our servers for some reasons. Please contact us in 5 days to fix this or '.THEME_PRETTY_NAME.' framework page will be blocked.<br> <span>State : ' . $this->state . '</span>';
		else
			echo 'The '.THEME_PRETTY_NAME.' page is <span>blocked</span> by TeslaThemes due to some <span>fraudulous action</span>.<br> Please contact us at hi@teslathemes.com or click the link below to correct your license if you think that this is a mistake. <br><span>State : ' . $this->state . '</span>';
